...
ThreeGppAntennaModel::GetTypeId()
{
    static TypeId tid = TypeId("ns3::ThreeGppAntennaModel")
                            ...
                            .AddAttribute("DowntiltDeg",
                                "Electrical downtilt angle in degrees",
                                DoubleValue(0.0),
                                MakeDoubleAccessor(&ThreeGppAntennaModel::SetDowntiltDeg,
                                                   &ThreeGppAntennaModel::GetDowntiltDeg),
...
ThreeGppAntennaModel::GetGainDb(Angles a)
{
    ...
    double thetaPrime = thetaDeg - m_downtiltDeg;

    // clamp to [0, 180]
    thetaPrime = std::max(0.0, std::min(180.0, thetaPrime));


    // 🔥 여기 추가
    // NS_LOG_UNCOND("[ThreeGppAntennaModel] m_downtiltDeg = " << m_downtiltDeg);
    // NS_LOG_UNCOND("[ThreeGppAntennaModel] thetaPrime = " << thetaPrime);


    NS_ASSERT_MSG(-180.0 <= phiDeg && phiDeg <= 180.0, "Out of boundaries: phiDeg=" << phiDeg);
    NS_ASSERT_MSG(0.0 <= thetaPrime && thetaPrime <= 180.0, "Out of boundaries: thetaPrime=" << thetaPrime);

    // compute the radiation power pattern using equations in table 7.3-1 in
    // 3GPP TR 38.901
    double vertGain = -std::min(m_slaV,
                                12 * pow((thetaPrime - 90) / m_verticalBeamwidthDegrees,
...
void
ThreeGppAntennaModel::SetDowntiltDeg(double angleDeg)
{
    m_downtiltDeg = angleDeg;
}

double
ThreeGppAntennaModel::GetDowntiltDeg() const
{
    return m_downtiltDeg;
}
