...
MmWaveRadioEnergyModelEnb::ChangeState (int state)
{
  ...
  // 현재 상태에 따른 기본 전류 기반 에너지
  double baseEnergy = duration.GetSeconds() * GetStateA(m_currentState) * supplyVoltage;

  // TxPower(dBm) 가져오기 (MmWaveEnbPhy에서)
  ...
  if (m_node)
  {
    for (uint32_t i = 0; i < m_node->GetNDevices(); ++i)
    {
      ...
      Ptr<mmwave::MmWaveEnbNetDevice> enbDev = DynamicCast<mmwave::MmWaveEnbNetDevice>(dev);
      if (enbDev)
      {
        Ptr<mmwave::MmWaveEnbPhy> phy = enbDev->GetPhy();
        if (phy)
        {
          txPowerDbm = phy->GetTxPower(); // dBm 단위
        }
        break;
      }
    }
  ...
  // dBm → Watt 변환
  double txPowerWatt = pow(10.0, (txPowerDbm - 30.0) / 10.0);

  // 송신 상태(Tx)일 때만 TxPower 반영
  double energyToDecrease = baseEnergy;
  if (m_currentState == 1)  // 1 == Tx 상태
  {
    energyToDecrease += duration.GetSeconds() * (10.0* txPowerWatt);
  }
