                                                    ...
                                                    Ptr<PhasedArrayModel> otherAntenna)
{
    ...
    // 2. AoA/AoD 계산
    // -------------------------------
    Angles angleTx(bPos, aPos);  // TX → RX 방향
    Angles angleRx(aPos, bPos);  // RX → TX 방향

    // -------------------------------
    // 3. RET 적용 (Electrical Downtilt)
    // -------------------------------
    double tiltRad = m_antenna->GetRetAngle() * M_PI / 180.0;

    angleTx.SetInclination(angleTx.GetInclination() + tiltRad);
    // RX에서는 tilt 적용하지 않아야 정상적인 시스템 (필요시 추가 가능)

    // 범위 제한
    auto clamp = [](double x) {
        if (x < 0) return 0.0;
        if (x > M_PI) return M_PI;
        return x;
    };
    angleTx.SetInclination(clamp(angleTx.GetInclination()));
    angleRx.SetInclination(clamp(angleRx.GetInclination()));

    // -------------------------------
    // 4. Steering Vector + RET → Beamforming Vector 생성
    // -------------------------------
    PhasedArrayModel::ComplexVector txBf =
        m_antenna->GetBeamformingVector(angleTx);

    PhasedArrayModel::ComplexVector rxBf =
        otherAntenna->GetBeamformingVector(angleRx);

    // -------------------------------
    // 5. 실제 장비에 BF 적용
    // -------------------------------
    m_antenna->SetBeamformingVector(txBf);
    otherAntenna->SetBeamformingVector(rxBf);
                                                    ...
                                                    Ptr<PhasedArrayModel> otherAntenna)
{
    ...
    // ====== [추가] RET: apply tilt phase ramp on TX BF vector ======
    double tiltRad = m_antenna->GetRetAngle() * M_PI / 180.0;
    auto &txBf = std::get<0>(bfVectors);   // transmitter BF vector

    for (uint64_t k = 0; k < txBf.GetSize(); k++)
    {
        double phase = tiltRad * k;  
        txBf[k] *= std::exp(std::complex<double>(0, phase));
    }
    // ==============================================================
                                                         ...
                                                         Ptr<PhasedArrayModel> otherAntenna)
{
    ...
    // ===== [추가] RET patch: apply tilt phase shift to TX codebook vector =====
    double tiltRad = m_antenna->GetRetAngle() * M_PI / 180.0;

    for (uint64_t k = 0; k < thisAntennaWeights.GetSize(); k++)
    {
        double phase = tiltRad * k; 
        thisAntennaWeights[k] *= std::exp(std::complex<double>(0, phase));
    }
    // ==========================================================================
