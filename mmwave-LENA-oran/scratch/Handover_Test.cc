...
//   LogComponentEnable ("E2Termination", LOG_LEVEL_LOGIC);
    ...
    // Set UE mobility - all start at (2000, 2000) and move in different directions at 50 m/s
    MobilityHelper uemobility;
    uemobility.SetMobilityModel("ns3::ConstantVelocityMobilityModel");
    uemobility.Install(ueNodes);

    // Starting position for all UEs
    double startX = 2000.0;
    double startY = 2000.0;
    double velocity = 50.0; // m/s

    // Set position and velocity for each UE
    for (uint32_t i = 0; i < ueNodes.GetN(); ++i) {
        ...
        cvMob->SetPosition(Vector(startX, startY, 1.5));
        
        // Calculate velocity vector based on angle (0°, 120°, 240°)
        double angle = i * 120.0 * M_PI / 180.0; // Convert to radians
        double vx = velocity * cos(angle);
        double vy = velocity * sin(angle);
        
        cvMob->SetVelocity(Vector(vx, vy, 0));
        
        NS_LOG_UNCOND("UE " << i+1 << " starting at (" << startX << ", " << startY 
                      << ") moving at " << velocity << " m/s at angle " << (i * 120.0) << " degrees");
    }
