...
// 안테나 모델 계층 (기본 및 3GPP 모델)
#include "ns3/antenna-model.h"
#include "ns3/three-gpp-antenna-model.h"     // ← 3GPP 방사 패턴 (RET 지원)
#include "ns3/isotropic-antenna-model.h"     // ← 기존 기본 등방성 모델
#include "ns3/phased-array-model.h"          // ← 배열형 안테나 (공통 기반 클래스)
#include "ns3/uniform-planar-array.h"        // ← 실제 UPA 구현 (DowntiltAngle 존재)
...
/*---------새롭게 바뀐 EnergyConsumptionPrint----------*/
void
EnergyConsumptionPrint (int nodeIndex)
{
  double currentTime = Simulator::Now().GetSeconds();
  double diffEnergy = totalnewEnergyConsumption_storage[nodeIndex] - totaloldEnergyConsumption_storage[nodeIndex];

  // 현재 셀의 순간(ΔE) 에너지 업데이트
  current_energy_consumption[nodeIndex] = diffEnergy;
  totaloldEnergyConsumption_storage[nodeIndex] = totalnewEnergyConsumption_storage[nodeIndex];

  // 모든 셀의 평균 계산 (num_of_mmdev은 전역변수로 이미 있음)
  double sumDiff = 0.0;
  int validCells = 0;
  for (int i = 0; i < num_of_mmdev; i++)
  {
    if (current_energy_consumption[i] > 0)
    {
      sumDiff += current_energy_consumption[i];
      validCells++;
    }
  }
  ...
  // 출력 형식: 각 셀별 ΔE + 전체 평균
  NS_LOG_UNCOND(
      "[Time " << std::fixed << std::setprecision(1) << currentTime
               << "s] Cell " << (nodeIndex + 2)
               << " Instant Energy = " << std::setprecision(4) << diffEnergy << " J"
  );

  // Optional: CSV 로깅도 가능 (원하면 아래 주석 해제)
  // std::ofstream out("energy_instant.csv", std::ios::app);
  // out << currentTime << "," << (nodeIndex+2) << "," << diffEnergy << "," << avgDiff << std::endl;
  // out.close();
}



void PrintUeAnglesAndGains(Ptr<MmWaveEnbNetDevice> enbDev, NodeContainer ueNodes)
{
    NS_LOG_UNCOND("========== UE Angle & Gain Debug ==========");

    Ptr<MobilityModel> enbMob = enbDev->GetNode()->GetObject<MobilityModel>();
    Vector enbPos = enbMob->GetPosition();

    // gNB의 Array 모델
    Ptr<PhasedArrayModel> array = enbDev->GetAntenna(0);  // CC 0 기준
    if (!array)
    {
        NS_LOG_UNCOND("ERROR: enbDev->GetAntenna(0) returned NULL");
        return;
    }

    Ptr<AntennaModel> elem = array->GetAntennaElement()->GetObject<AntennaModel>();
    Ptr<ThreeGppAntennaModel> threeGpp = DynamicCast<ThreeGppAntennaModel>(elem);
    

    if (!threeGpp)
    {
        NS_LOG_UNCOND("ERROR: AntennaElement is NOT ThreeGppAntennaModel!");
        return;
    }
    ...
    for (uint32_t u = 0; u < ueNodes.GetN(); u++)
    {
        ...
        // gNB → UE 방향 각도
        Angles ang(uePos, enbPos);

        double phiDeg   = RadiansToDegrees(ang.GetAzimuth());
        double thetaDeg = RadiansToDegrees(ang.GetInclination());

        // 3GPP Gain(dB)
        double gainDb = threeGpp->GetGainDb(ang);
    ...
    NS_LOG_UNCOND("AntennaElement TypeId = " 
      << elem->GetInstanceTypeId().GetName());
    NS_LOG_UNCOND("===========================================");
}


void
RsrpSinrCallback (uint64_t imsi,
                  uint16_t cellId,
                  double rsrp,   // or maybe in dBm, 확인 필요
                  double sinr)   // dB
{
  NS_LOG_UNCOND ("[RSRP/SINR] t="
...
/*----------파라미터 세팅하는 부분 ----------*/ 
static ns3::GlobalValue g_txPower(
  "TXP", "Tx Power of gNB in dBm",
  ns3::DoubleValue(40.0), 
  ns3::MakeDoubleChecker<double>(0, 60));

  
// === [추가] RET (Electrical Downtilt) 파라미터 ===
static ns3::GlobalValue g_retAngle(
  "RET", "Electrical downtilt angle in degrees (RET)",
  ns3::DoubleValue(0.0), // 기본값: 0도
  ns3::MakeDoubleChecker<double>(-30.0, 30.0)); // 허용 범위: -30° ~ 30°
...
static ns3::GlobalValue g_simTime("simTime", "Simulation time in seconds", ns3::DoubleValue(10),
...
main(int argc, char *argv[]) {
    ...
    LogComponentEnable("PhasedArrayModel", LOG_LEVEL_INFO);
    ...
    // === [추가] g_txPower 전역 파라미터 읽기 및 적용 ===
    DoubleValue txp;
    GlobalValue::GetValueByName("TXP", txp);
    NS_LOG_UNCOND("Applying TXP = " << txp.Get() << " dBm");

    Config::SetDefault("ns3::LteEnbPhy::TxPower", DoubleValue(txp.Get()));
    Config::SetDefault("ns3::MmWaveEnbPhy::TxPower", DoubleValue(txp.Get()));
    // ===============================================


    // === [추가] g_retAngle 전역 다운틸트 파라미터 읽기 및 적용 ===
    DoubleValue retDeg;
    GlobalValue::GetValueByName("RET", retDeg);
    NS_LOG_UNCOND("Applying RET (Electrical Downtilt) = " << retDeg.Get() << " degrees");

    // 1) PhasedArrayModel 자체에 보관하는 RET (네가 이미 쓰는 것)
    Config::SetDefault("ns3::PhasedArrayModel::RetAngle", DoubleValue(retDeg.Get()));

    // 2) 실제 AntennaElement 객체를 만들 때, DowntiltDeg를 직접 세팅
    Ptr<ThreeGppAntennaModel> elem = CreateObject<ThreeGppAntennaModel>();
    elem->SetAttribute("DowntiltDeg", DoubleValue(retDeg.Get()));
    Config::SetDefault("ns3::PhasedArrayModel::AntennaElement", PointerValue(elem));
    ...
    int numAntennasMmWave = 16; //uintegerValue.Get();
    ...
    // 2) beamforming 모델 설정 — 반드시 gNB 설치 전에
    mmwaveHelper->SetBeamformingModelType("ns3::MmWaveDftBeamforming");
    // 3) PhasedArrayModel에 RET(Tilt) 설정
    mmwaveHelper->SetEnbPhasedArrayModelAttribute("RetAngle", DoubleValue(retDeg.Get()));
    ...
    // speed->SetAttribute("Min", DoubleValue(30.0));
    ...
    // -------------------------------------
    // FIX: UE fixed positions (E/W/S/N × 10/30/50/70/100m)
    // -------------------------------------
    std::vector<double> distances = {10, 30, 50, 70, 100};
    ...
    {
        // East
        uePositionAlloc->Add(Vector(centerPosition.x + d, centerPosition.y, 1.5));
    }
    ...
    // -------------------------------------------------------------
    // MUST BE HERE: enbDev 선언
    // -------------------------------------------------------------
    Ptr<MmWaveEnbNetDevice> enbDev = mmWaveEnbDevs.Get(0)->GetObject<MmWaveEnbNetDevice>();
    
    // ================== RET DEBUG ==================
    Ptr<PhasedArrayModel> array = enbDev->GetAntenna(0);
    
    NS_LOG_UNCOND("------------- RET DEBUG -------------");
    
    if (array)
    {
        NS_LOG_UNCOND("[CHECK-RET] PhasedArrayModel::RetAngle = "
                      << array->GetRetAngle());
    }
    else
    {
        NS_LOG_UNCOND("[CHECK-RET] ERROR: Antenna array is NULL");
    }
    
    Ptr<const AntennaModel> elem2 = array->GetAntennaElement();
    NS_LOG_UNCOND("[CHECK-RET] AntennaElement Type = "
                  << elem2->GetInstanceTypeId().GetName());
    
    NS_LOG_UNCOND("-------------------------------------");
    // ===============================================
    
    // UEs install AFTER this
    NetDeviceContainer mcUeDevs = mmwaveHelper->InstallMcUeDevice(ueNodes);
    
    // UE Gain 출력
    PrintUeAnglesAndGains(enbDev, ueNodes);
  ...
  //     "/NodeList/*/DeviceList/*/ReportCurrentCellRsrpSinr",
  ...
  // On the remoteHost there is UDP OnOff Application
    ...
    if (run) {
        ...
        Simulator::Run();
        PrintUeAnglesAndGains( mmWaveEnbDevs.Get(0)->GetObject<MmWaveEnbNetDevice>(), ueNodes );
