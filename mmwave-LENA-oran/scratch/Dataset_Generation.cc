...
static ns3::GlobalValue g_txPower(
  ...
  ns3::DoubleValue(40.0), ns3::MakeDoubleChecker<double>(0, 60));
...
//   LogComponentEnable ("E2Termination", LOG_LEVEL_LOGIC);
    ...
    speed->SetAttribute("Min", DoubleValue(100.0));
    speed->SetAttribute("Max", DoubleValue(300.0));
  ...
  // On the remoteHost there is UDP OnOff Application
    ...
    Ptr <LteHelper> lteTraceHelper = CreateObject<LteHelper>();
    lteTraceHelper->Initialize();
    lteTraceHelper->EnablePhyTraces();
    lteTraceHelper->EnableMacTraces();
