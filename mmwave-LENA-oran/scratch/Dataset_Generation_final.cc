...
static ns3::GlobalValue g_retAngle(
  ...
  ns3::DoubleValue(8.0), // 기본값: 8도
  ns3::MakeDoubleChecker<double>(-15.0, 30.0)); // 허용 범위: -15° ~ 30°


// === [추가] TTT (Time To Trigger) 파라미터 ===E
static ns3::GlobalValue g_tttValue(
  "TTT",     "Time To Trigger for handover in milliseconds",
  ns3::DoubleValue(256.0),  // 기본값: 256 ms (3GPP 표준값)
  ns3::MakeDoubleChecker<double>(0.0, 5120.0)); // 허용 범위: 0 ~ 5120 ms


// === [추가] HYS (Hysteresis) 파라미터 ===
static ns3::GlobalValue g_hystValue(
  "HYS",     "Hysteresis for handover decision in dB",
  ns3::DoubleValue(3.0),    // 기본값: 3 dB
  ns3::MakeDoubleChecker<double>(0.0, 15.0)); // 허용 범위: 0 ~ 15 dB
...
//   LogComponentEnable ("E2Termination", LOG_LEVEL_LOGIC);
    ...
    // === [추가] TTT와 HYS 파라미터 읽기 ===
    DoubleValue tttParam, hystParam;
    GlobalValue::GetValueByName("TTT", tttParam);
    GlobalValue::GetValueByName("HYS", hystParam);
    double ttt = tttParam.Get();
    double hys = hystParam.Get();
    // =======================================
    ...
    // === [추가] RET (Electrical Downtilt) 파라미터 적용 ===
    DoubleValue retDeg;
    GlobalValue::GetValueByName("RET", retDeg);
    double retAngleRad = retDeg.Get() * M_PI / 180.0; // degrees → radians

    NS_LOG_UNCOND("Applying RET = " << retDeg.Get() << " deg ("
                                    << retAngleRad << " rad)");

    // UPA (UniformPlanarArray)에 DowntiltAngle 적용
    Config::SetDefault("ns3::UniformPlanarArray::DowntiltAngle",
                      DoubleValue(retAngleRad));
    // ===============================================


    // === [추가] TTT (Time To Trigger) 파라미터 적용 ===
    DoubleValue tttMs;
    GlobalValue::GetValueByName("TTT", tttMs);

    NS_LOG_UNCOND("Applying TTT = " << tttMs.Get() << " ms");

    // A3RsrpHandoverAlgorithm의 TimeToTrigger 설정 (mmwave-LENA 표준 방식)
    Config::SetDefault("ns3::A3RsrpHandoverAlgorithm::TimeToTrigger",
                      TimeValue(MilliSeconds(tttMs.Get())));
    // ===============================================


    // === [추가] HYS (Hysteresis) 파라미터 적용 ===
    DoubleValue hystDb;
    GlobalValue::GetValueByName("HYS", hystDb);

    NS_LOG_UNCOND("Applying HYS = " << hystDb.Get() << " dB");

    // A3RsrpHandoverAlgorithm의 Hysteresis 설정 (mmwave-LENA 표준 방식)
    Config::SetDefault("ns3::A3RsrpHandoverAlgorithm::Hysteresis",
                      DoubleValue(hystDb.Get()));
    // ===============================================
    ...
    // === [추가] 핸드오버 알고리즘 설정 (TTT, HYS 적용을 위한 A3Rsrp 사용) ===
    // mmwave-LENA 문서에 따른 표준 방식: A3RsrpHandoverAlgorithm 사용
    // TTT와 HYS 파라미터가 이 알고리즘에 적용됨
    mmwaveHelper->SetLteHandoverAlgorithmType("ns3::A3RsrpHandoverAlgorithm");
    // 위에서 설정한 TTT와 HYS 값이 A3RsrpHandoverAlgorithm에 자동으로 적용됨
    // ================================================================
