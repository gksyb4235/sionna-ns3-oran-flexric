...
try:
    ...
    from .oai_panel import oai_kpi_panel, oai_cir_panel, oai_control_panel
    _OAI_AVAILABLE = True
except ImportError:
...
class SionnaRtGui:
    def __init__(self, cfg: GuiConfig):
        ...
        # On WSL2, CUDA-OpenGL interop is not supported, so force rasterization.
        import platform
        _is_wsl2 = "microsoft" in platform.uname().release.lower()
        if "cuda" in mi.variant() and not _is_wsl2:
            self.set_use_denoiser(self.cfg.rendering.use_denoiser)
        else:
            # If there is no CUDA-compatible GPU or running on WSL2, switch to rasterization (cheaper).
    ...
    def tick(self):
        ...
        # ── OAI ZMQ Bridge 처리 (매 프레임) ──────────────────────
        if self.zmq_bridge is not None:
            self.zmq_bridge.poll_and_apply(self)
    ...
    def add_radio_device(
        ...
        elif self.scene.get(name) is not None:
            raise ValueError(f'A radio device named "{name}" already exists')
    ...
    def gui(self):
        ...
        # ── OAI 연동 패널 (Animation 바로 아래) ─────────────────────
        if _OAI_AVAILABLE:
            oai_control_panel(self, self.zmq_bridge)
            oai_kpi_panel(self, self.zmq_bridge)
            oai_cir_panel(self)
