...
public:
    struct UeRadioState
    {
        ...
        uint32_t servingGnbNodeId{0};
        bool active{true};
    };

    using UeRadioStateProvider =
        std::function<std::optional<UeRadioState>(uint32_t, uint32_t)>;
    ...
    /**
     * Provide current ns-3 RRC identity for a channel request's endpoint pair.
     * The callback is optional so the upstream ns3sionna server remains usable.
     */
    void SetUeRadioStateProvider(UeRadioStateProvider provider);
    std::optional<UeRadioState> GetUeRadioState(uint32_t nodeA, uint32_t nodeB) const;
...
private:
    ...
    UeRadioStateProvider m_ueRadioStateProvider;
