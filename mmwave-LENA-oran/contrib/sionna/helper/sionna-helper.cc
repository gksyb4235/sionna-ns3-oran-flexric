...
void
SionnaHelper::SetUeRadioStateProvider(UeRadioStateProvider provider)
{
    m_ueRadioStateProvider = std::move(provider);
}

std::optional<SionnaHelper::UeRadioState>
SionnaHelper::GetUeRadioState(uint32_t nodeA, uint32_t nodeB) const
{
    if (!m_ueRadioStateProvider)
    {
        return std::nullopt;
    }
    return m_ueRadioStateProvider(nodeA, nodeB);
}
...
SionnaHelper::SetChannelBandwidth(int channel_bw)
{
    ...
    const double channelBwHz = channel_bw * 1e6;
    double Nt = BOLTZMANN * 293 * channelBwHz;
    // receiver noise Floor (W) which accounts for thermal noise and non-idealities of the receiver
    const double noiseFigureDb = 5.0;
    const double noiseFigureLinear = std::pow(10.0, noiseFigureDb / 10.0);
    double noiseFloor = noiseFigureLinear * Nt;
...
SionnaHelper::Start()
{
    ...
    for (auto iter = c.Begin(); iter != c.End(); ++iter)
    {
        ...
        if (mobilityModel)
        {
            ...
            else if (sionnaMobilityModel->GetModel() == "External")
            {
                auto* external_model = node_info->mutable_external_position_model();
                auto* position_vector = external_model->mutable_position();
                position_vector->set_x(position.x);
                position_vector->set_y(position.y);
                position_vector->set_z(position.z);
                external_model->set_active(sionnaMobilityModel->IsActive());
            }
            else
            {
