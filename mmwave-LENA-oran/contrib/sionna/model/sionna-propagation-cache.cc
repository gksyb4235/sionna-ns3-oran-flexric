...
SionnaPropagationCache::GetPropagationData(Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
    ...
    const Vector currentPosA = a->GetPosition();
    const Vector currentPosB = b->GetPosition();

    // Activity can change without a position update. Invalidate the cache so
    // the lifecycle transition is forwarded to Sionna/GUI on the next lookup.
    ...
    for (auto node = allNodes.Begin(); node != allNodes.End(); ++node)
    {
        Ptr<SionnaMobilityModel> mobility =
            DynamicCast<SionnaMobilityModel>((*node)->GetObject<MobilityModel>());
        if (!mobility || !mobility->IsExternal())
        {
            continue;
        }
        const auto [it, inserted] =
            m_lastExternalActive.emplace((*node)->GetId(), mobility->IsActive());
        if (!inserted && it->second != mobility->IsActive())
        {
            it->second = mobility->IsActive();
    ...
    if (m_caching)
    {
        ...
        if (it != m_cache.end())
        {
            ...
            for (CacheEntry c_entry : it->second) {
                ...
                const Vector cachedPosA = node_a->GetId() == c_entry.m_a
                                              ? c_entry.m_a_position
                                              : c_entry.m_b_position;
                const Vector cachedPosB = node_b->GetId() == c_entry.m_b
                                              ? c_entry.m_b_position
                                              : c_entry.m_a_position;
                if (c_entry.m_end_time >= current_time &&
                    c_entry.m_start_time <= current_time && cachedPosA == currentPosA &&
                    cachedPosB == currentPosB)
                {
    ...
    const auto ueRadioState =
        m_sionnaHelper->GetUeRadioState(node_a->GetId(), node_b->GetId());
    if (ueRadioState)
    {
        auto* state = propagation_request->mutable_ue_radio_state();
        state->set_node_id(ueRadioState->nodeId);
        state->set_imsi(ueRadioState->imsi);
        state->set_rnti(ueRadioState->rnti);
        state->set_serving_cell_id(ueRadioState->servingCellId);
        state->set_serving_gnb_node_id(ueRadioState->servingGnbNodeId);
        state->set_active(ueRadioState->active);
    }
    ...
    for (auto node = allNodes.Begin(); node != allNodes.End(); ++node)
    {
        ...
        auto* state = propagation_request->add_external_node_states();
        state->set_node_id((*node)->GetId());
        ...
        state->set_active(mobility->IsActive());
    }
