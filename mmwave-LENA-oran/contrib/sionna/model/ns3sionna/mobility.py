...
class ExternalMobility(ConstantMobility):
    """Position and lifecycle supplied by ns-3, typically from a SUMO trace."""
    def __init__(self, node_id, pos, active=True, collect_history=True):
        super(ExternalMobility, self).__init__(node_id, pos, collect_history)
        self.active = active

    def update_external(self, sim_time, pos, active):
        pos = np.array(pos)
        dt_seconds = (sim_time - self.last_update) / 1e9
        velocity = np.zeros(3)
        if dt_seconds > 0:
            velocity = (pos - self.pos) / dt_seconds
        self.update_pos(sim_time, pos, velocity, False)
        self.active = active
