...
class SionnaEnv:
    ...
    def __init__(self, model_folder='./models/', rt_fast=False, default_mode=MODE_P2P, rt_max_parallel_links=256, est_csi=True, VERBOSE=True,
        ...
        # Sionna RT 2.x runs on Mitsuba/Dr.Jit, not TensorFlow. Query the
        # backend that actually performs ray tracing so the status reflects
        # real RT acceleration and does not pull TensorFlow into this server.
        self.has_cuda = bool(dr.has_backend(dr.JitBackend.CUDA))

        if self.has_cuda:
            print(f"Sionna RT GPU backend detected (Mitsuba variant: {mi.variant()})")
        else:
            print(f"Using Sionna RT CPU backend (Mitsuba variant: {mi.variant()})")
    ...
    def _link_metrics(self, tau, h, tx_pos, rx_pos):
        """Return a finite ns-3 channel even when Sionna finds no propagation path."""
        tau_values = np.asarray(tau).reshape(-1)
        valid_tau = tau_values[np.isfinite(tau_values) & (tau_values >= 0)]
        power = float(np.mean(np.abs(h) ** 2)) if np.size(h) else 0.0

        if valid_tau.size == 0 or not np.isfinite(power) or power <= 0.0:
            distance = float(np.linalg.norm(np.asarray(rx_pos) - np.asarray(tx_pos)))
            delay_ns = int(round(distance / 299_792_458.0 * 1e9))
            normalized = np.ones(len(self.frequencies), dtype=np.complex64)
            if self.VERBOSE:
                print(f"No RT path found; using {self.NO_PATH_LOSS_DB:.0f}dB loss "
                      f"and geometric delay {delay_ns}ns")
            return delay_ns, self.NO_PATH_LOSS_DB, normalized

        delay_ns = int(round(np.min(valid_tau) * 1e9, 0))
        loss_db = float(-10 * np.log10(power))
        return delay_ns, loss_db, h / np.sqrt(power)
    ...
    def compute_cfr(self, csi_req, reply_wrapper):
        ...
        self._apply_external_node_states(csi_req)
    ...
    def compute_cfr_with_lookahead(self, csi_req, reply_wrapper):
        ...
        for lah_time_idx, lah_time in enumerate(lah_time_vec): # iterate over time
            ...
            for curr_rx_id, curr_rx_node in enumerate(rx_nodes):
                ...
                lnk_delay, lnk_loss, h_normalized = self._link_metrics(
                    tau[rx_id, :, :, :, :], h, tx_pos,
                    self.node_info[curr_rx_node].get_pos_at(lah_time))
    ...
    def _walk(self, node_id, dt):
        ...
        # External mobility is already advanced by ns-3/SUMO. Do not extrapolate
        # it again in the Sionna process.
        if isinstance(self.node_info[node_id], ExternalMobility):
            return
    ...
    def _compute_cfr_via_position(self, req_sim_time, tx_node, rx_node, req_mode):
        ...
        for rx_id, curr_rx_node in enumerate(rx_nodes):
            h = np.squeeze(h_raw[rx_id, :, :, :, :, ])
            lnk_delay, lnk_loss, h_normalized = self._link_metrics(
                tau[rx_id, :, :, :, :], h, self.node_info[tx_node].pos,
                self.node_info[curr_rx_node].pos)
    ...
    def _init_mobility(self, sim_init_msg):
        ...
        for node_info in sim_init_msg.nodes:
            ...
            elif (node_info.HasField("external_position_model")):
                model = node_info.external_position_model
                pos = model.position
                self.node_info[node_info.id] = ExternalMobility(
                    node_info.id, [pos.x, pos.y, pos.z], model.active)
    ...
    def _apply_external_node_states(self, csi_req):
        for state in csi_req.external_node_states:
            mobility = self.node_info.get(state.node_id)
            if not isinstance(mobility, ExternalMobility):
                continue
            pos = state.position
            mobility.update_external(csi_req.time, [pos.x, pos.y, pos.z], state.active)


    def run(self, bind_endpoint="tcp://*:5555"):
        '''
        ...
        print(f"Sionna server socket ready at {bind_endpoint} ...")
